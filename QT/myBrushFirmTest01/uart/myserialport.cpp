...
qint64 MySerialPort::Send(const QByteArray &data) {
...
void MySerialPort::SetPortName(const QString &name) {
    ...
    port_info_ = QSerialPortInfo(name);
    serial_port_p->setPortName(name);
  //  serial_port_p->setPortName("com1");//"/dev/ttys002"
    serial_port_p->setFlowControl(QSerialPort::NoFlowControl);

}

QString MySerialPort::GetPortName() {
    return serial_port_p->portName();
}
...
// 扫描可用串口
QStringList MySerialPort::scanSerial()
{
    ...
    foreach(const QSerialPortInfo &info, QSerialPortInfo::availablePorts())
    {
        serial_port_p->setPort(info);
        // 判断端口能否以读写方式打开
        if(serial_port_p->open(QIODevice::ReadWrite))
        {
            serialStrList.append(serial_port_p->portName());
            serial_port_p->close(); // 然后自动关闭等待人为开启（通过"打开串口按钮"）
        }
    }
