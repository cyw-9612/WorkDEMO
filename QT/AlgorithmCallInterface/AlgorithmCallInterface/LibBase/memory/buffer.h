...
public:
    ...
    void WriteBytes(size_t n) {
     ...
     bool findMark(int offset, int size, int startIndex, const char* markBytes)
     {
         if (length() == 0)
         {
             return false;
         ...
         if (offset < 0/* || offset > static_cast<int>(length() - 1)*/)
         {
             return false;
         }
         ...
         int markLen = strlen(markBytes);
         int maxIndex = offset + size - markLen; //确定最大下标
         if (startIndex < offset || startIndex > maxIndex)
         {
             return false;
         }

         if (markBytes == nullptr || markLen == 0)
         {
             return false;
         }

         if (size < markLen) //没有可比对的数据
         {
             return false;
         }
         ...
         {
             if ( *(buffer_ + startIndex + i) != markBytes[i])
             {
                 equal = false;
             }
     ...
     int findMark(int offset, int size, const char* markBytes)
     {
         int indexOffset = -1;
         int count = length() - strlen(markBytes);
         for (int i = offset; i < count; i++)
         {
             if (findMark(offset, size, i, markBytes))
             {
                 indexOffset = i;
                 break;
             }
...
private:
    char* buffer_;// 缓存
    size_t capacity_;// 容量
    size_t read_index_;// 可读字节序号
    size_t write_index_;// 可写字节序号
    size_t reserved_prepend_size_;//保留字节数
